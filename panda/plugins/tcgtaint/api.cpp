...
void tcgtaint_taint_register(bool istmp, unsigned char regno, int label) {
    tcgtaint_ctx.taint_engine->setTaintedRegister(label, RegisterKind(istmp),
                                                  regno);
}

void tcgtaint_taint_virtual_memory(target_ulong addr, unsigned int size,
                                   int label) {
...
void tcgtaint_taint_physical_memory(hwaddr phyaddr, unsigned int size,
                                    int label) {
    tcgtaint_ctx.taint_engine->setTaintedMemory(label, phyaddr, size);
}
...
bool tcgtaint_is_taint_enabled() { return taint_is_user_enabled; }
...
size_t tcgtaint_get_physical_memory_labels_count(target_ulong addr) {
    const std::set<int> *memoryLabels =
        tcgtaint_ctx.taint_engine->getMemoryLabels(addr);
    if (memoryLabels) {
