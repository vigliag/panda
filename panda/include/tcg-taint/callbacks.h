...
typedef void (*notify_taint_moveR2R_t)(bool srctmp, target_ulong src,
                                       bool dsttmp, target_ulong dst);
extern notify_taint_moveR2R_t notify_taint_moveR2R;

/* Micro ops */
typedef void (*notify_taint_micro_ld_t)(target_ulong reg, target_ulong addr,
                                        uint32_t size);
extern notify_taint_micro_ld_t notify_taint_micro_ld;

typedef void (*notify_taint_micro_st_t)(target_ulong reg, target_ulong addr,
                                        uint32_t size);
...
/* Set taint label */
// typedef void (*notify_taint_memory_t)(target_ulong addr, unsigned int size,
// int label);  extern notify_taint_memory_t notify_taint_memory;

// typedef void (*notify_taint_register_t)(bool istmp, unsigned char regno, int
// label);  extern notify_taint_register_t notify_taint_register;
